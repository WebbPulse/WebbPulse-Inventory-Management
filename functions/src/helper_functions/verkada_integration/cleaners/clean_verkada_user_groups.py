...
def clean_verkada_user_groups(org_id, verkada_bot_user_info):
    ...
    try:
        ...
        if not verkada_integration_ref.exists:
            logging.warning(f"Verkada integration settings not found for organization {org_id}.")
            return
        org_verkada_user_groups = verkada_integration_ref.to_dict().get('orgVerkadaUserGroups', [])
        if not org_verkada_user_groups:
            logging.warning(f"No groups found for organization {org_id}.")
            return
        ...
        logging.info(f"Cleaned up user groups for organization {org_id}.")
    
    except Exception as e:
        logging.error(f"Error cleaning up user groups for organization {org_id}: {str(e)}")
...
def remove_group(verkada_bot_user_info, group):
    ...
    try:
        ...
        logging.error(f"Failed to remove group {group_id} from Verkada: {e}")
        return
    except Exception as e:
        # Handle any other exceptions
        logging.error(f"An unexpected error occurred while removing group {group_id}: {e}")
        return